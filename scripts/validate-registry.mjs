...
function validateProfiles(profiles, skillMap) {
  ...
  for (const [id, profile] of profiles.entries()) {
    ...
    for (const skillId of profile.localSkills || []) {
      ...
      if (!fs.existsSync(path.join(dir, 'SKILL.md'))) fail(`${id}: Local Skill 缺少 SKILL.md：${skillId}`);
      if (!fs.existsSync(path.join(dir, 'agents', 'openai.yaml'))) warn(`${id}: Local Skill 缺少 agents/openai.yaml：${skillId}`);
    }
  }
